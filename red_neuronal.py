...
model=keras.models.Sequential([keras.layers.Dense(10,activation=tf.nn.relu,input_shape=(input_dim,)),keras.layers.Dense(10,activation=tf.nn.relu), keras.layers.Dense(output_dim)])
model.compile(optimizer='adam', loss='mean_squared_error', metrics=['mean_absolute_percentage_error'])
model.summary()

validation_split=0.2
history=model.fit(X,Y,workers=4,epochs=150,verbose=2,validation_split=validation_split)
